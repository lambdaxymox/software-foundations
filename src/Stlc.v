...
Proof.
  ...
  - destruct (beq_id x0 i) eqn : Heq. 
    ...
    apply beq_id_false_iff in Heq. auto.
  - inversion H; subst. rewrite <- beq_id_refl. auto.
    apply beq_id_false_iff in H1. rewrite H1. auto.
  - eapply s_app; try (apply IHt1); try (apply IHt2); reflexivity.
  - inversion H; subst; auto.
    apply IHt1 in H2; subst; apply IHt2 in H4; subst; auto.
  - destruct (beq_id x0 i) eqn : Heq. subst; auto.
    + apply beq_id_true_iff in Heq; subst; constructor.
    + apply beq_id_false_iff in Heq; subst; constructor. auto.
      apply IHt. auto.
  - inversion H; subst. rewrite <- beq_id_refl. auto.
    apply beq_id_false_iff in H4. rewrite -> H4. 
...
Proof. 
  ...
  apply T_App with TBool. apply T_Var. apply update_eq.
  apply T_Var. Search update.
  apply update_neq. unfold not. intros. inversion H.
Qed.
(** [] *)
...
Proof with auto. 
  ...
  econstructor.
  constructor.
  apply T_App with (TBool). constructor. constructor.
  apply T_App with (TBool). constructor. constructor.
      ...
      (tabs x (TArrow TBool TBool)
         (tabs y (TArrow TBool TBool)
            (tabs z TBool
               (tapp (tvar y) (tapp (tvar x) (tvar z)))))) \in
      T.
Proof. repeat (try econstructor); repeat (try constructor). Qed.
(** [] *)
