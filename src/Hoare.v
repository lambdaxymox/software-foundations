...
(**
Let a = X + 1. We can choose this expression because arithmetic expressions
quantify over variable names as well as numbers in IMP. To show that hoare_asgn_wrong
is incorrect, we simply apply it. Since the precondition is trivially try, we enter the
commant block and assign X + 1 to X. In the postcondition, we have X = X + 1, which is
absurd.
*)
(** [] *)
...
Proof.
  intros F m a P. unfold hoare_triple. intros st st' H0 H1.
  destruct H1. inversion H0; subst.
  assert (t_update (t_update st X (aeval st a)) X (st X) = st).
  { apply F. intros x. rewrite -> t_update_shadow. 
    rewrite t_update_same. reflexivity. }
  rewrite -> H1. rewrite -> t_update_eq. 
...
Theorem hoare_asgn_fwd_exists :
  ...
  {{fun st => exists m, P (t_update st X m) /\
                st X = aeval (t_update st X m) a }}.
Proof.
  intros F a P. unfold hoare_triple. intros. exists (st X).
  apply hoare_asgn_fwd with (m := (st X)) (a := a) (P := P) in F.
  unfold hoare_triple in F. apply F with st.
...
Proof.
  ...
  intros st H0. unfold assn_sub. rewrite t_update_eq. 
  simpl. assumption.
Qed.
...
Theorem assn_sub_ex1'' : 
  ...
  {{ fun st => st X <= 5 }}.
Proof.
  remember (fun st : state => st X + 1 <= 5) as P.
  remember (fun st : state => st X <= 5) as P'.
  remember P' as Q.
  assert(H0 : forall st : state, st X <= st X + 1).
  { intros. omega. }
  assert(H1 : P ->> P').
  { intros st HP. admit. }
  apply hoare_consequence_pre with (P' := P').
  unfold hoare_triple. intros st st' H HP'. rewrite <- HeqQ in HP'.
  rewrite -> HeqP' in HP'. 
Abort.
...
Proof.
  eapply hoare_consequence_pre.
  apply hoare_asgn. unfold assert_implies.
  intros st H035. unfold assn_sub. rewrite -> t_update_eq. 
  simpl. assumption.
Qed.
(** [] *)
...
Proof.
  ...
  apply hoare_asgn.
  apply hoare_asgn.
  intros st st' H HstXY.
  inversion H; subst.
  unfold assn_sub. simpl.
  remember (t_update st Z (st Y)) as mz.
  rewrite -> t_update_eq. rewrite -> t_update_neq.
  rewrite -> t_update_eq.
  subst.
  rewrite -> t_update_neq. rewrite -> t_update_neq. rewrite -> t_update_eq. 
  assumption.
...
(*
Let a = AId X. 
Then aeval st a = aeval st (AId X) = X, so that when n <> 3,
st X = n <> 3.

For example, if we introduct n = 4 above, with aeval st X = 4,
then we cannot apply rule hoare_asgn since st X = 4 <> 3.
*)
(** [] *)
...
Proof.
  ...
  - eapply hoare_consequence_pre. 
    apply hoare_asgn.
    unfold bassn, assn_sub, t_update, assert_implies.
    simpl. intros st [_ H]. apply le_plus_minus.
