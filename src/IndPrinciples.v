...
Definition Pcomm (n : nat) (m : nat) : Prop := n + m = m + n.

Definition Pzero_l (n : nat) : Prop := n + 0 = n.

Definition Pzero_r (n : nat) : Prop := 0 + n = n.

Theorem plus_n_OP : forall n : nat, Pzero_l n.
Proof.
  apply nat_ind. reflexivity. intros n H. unfold Pzero_l. 
  simpl. rewrite -> H. reflexivity.
Qed.

Theorem plus_O_nP : forall n : nat, Pzero_r n.
Proof. 
  apply nat_ind. 
  - unfold Pzero_r. simpl. reflexivity.
  - intros n H. unfold Pzero_r. simpl. reflexivity.
Qed.

Theorem plus_commP : forall (n m : nat), Pcomm n m.
Proof.
  intros n. apply nat_ind.
  - unfold Pcomm. rewrite -> plus_n_OP. rewrite -> plus_O_nP. reflexivity.
  - intros n0 P. unfold Pcomm. rewrite <- plus_n_Sm. simpl. rewrite <- P.
    reflexivity.
Qed.

Theorem plus_assocP : forall (n m p : nat), Passoc n m p.
Proof.
  intros n m. apply nat_ind.
  - unfold Passoc. rewrite -> plus_n_OP. rewrite -> plus_n_OP. reflexivity.
  - unfold Passoc. intros n0 IHn0. rewrite <- plus_n_Sm. rewrite -> IHn0.
