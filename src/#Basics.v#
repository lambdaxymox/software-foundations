...
Proof.
  ...
  (* rewrite the goal from left to right. *)  
  rewrite -> H.
  (* rewrite the goal from right to left. *)
  (* rewrite <- H *)
  reflexivity.  
Qed.
...
Lemma andb_false_eq_false : forall b : bool, andb b false = false.
Proof.
  intros b. destruct b.
  - simpl. reflexivity.
  - simpl. reflexivity.
...
Lemma andb_false_eq_false' : forall b : bool, false = andb b false.
Proof.
  intros b. rewrite -> andb_false_eq_false. reflexivity.
Qed.

Theorem andb_true_elim2 : forall b c : bool,
  andb b c = true -> c = true.
Proof.
  intros b c. intros H. destruct c.
  - reflexivity.
  - rewrite <- H. rewrite -> andb_false_eq_false. reflexivity.
Qed.
(** [] *)
...
Lemma beq_commute : forall (a b : bool), a = b -> b = a.
Proof.
  intros a b. intros H. destruct a.
  - destruct b.
    + reflexivity. 
...
Lemma orb_true_eq_true : forall (b : bool), orb true b = true.
Proof.
  intros b. destruct b.
  - reflexivity.
  - reflexivity.
...
Lemma orb_false_eq_b : forall (b : bool), orb false b = b.
Proof.
  intros b. destruct b.
  - reflexivity.
  - reflexivity.
...
Lemma andb_true_eq_c: forall (c : bool), andb true c = c.
Proof.
  intros c. destruct c.
  - reflexivity.
  - reflexivity.
...
Proof.
  intros b c. 
  destruct b.
  - rewrite -> orb_true_eq_true. 
    rewrite -> andb_true_eq_c.
    intros H. 
    destruct c.
    + reflexivity.
    + rewrite -> H. reflexivity.
...
Inductive bin : Type :=
  | BZ    : bin         (* Binary Zero *)
  | Twice : bin -> bin  (* Even Number *)
  | More  : bin -> bin. (* Odd Number  *)
...
(* inc 0 = 2 * 0 + 1 = 1. *)
Example test_bin_incr1 : inc BZ = More BZ.
Proof. reflexivity. Qed.

(* inc 0 = 1. *)
Example test_bin_incr2 : inc (BZ) = More (BZ).
Proof. reflexivity. Qed.

(* inc (2 * 0) = 1. *)
Example test_bin_incr3 : inc (Twice BZ) = More BZ.
Proof. reflexivity. Qed.

(* 2 * (2 * (2 * 0 + 1) + 1) + 1) = 2 * 2 * 2 *(2*0 + 1) = 8 *)
Example test_bin_incr4 : 
  inc (More (More (More BZ))) = Twice (Twice (Twice (More BZ))).
Proof. reflexivity. Qed.

(* 0 + 1 + 1 + 1 + 1 + 1 = 2 * (2 * (0 + 1)) + 1 = 5 *)
Example test_bin_incr5 :
  inc (inc (inc (inc (inc BZ)))) = More (Twice (More BZ)).
Proof. simpl. reflexivity. Qed.
