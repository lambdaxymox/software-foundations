...
Proof.
  ...
  generalize dependent y2.
  induction Hy1;
  intros y2 Hy2; inversion Hy2; subst; try solve_by_invert.
  - reflexivity.
  - apply IHHy1 in H2. rewrite H2. reflexivity.
...
Proof.
  ...
  split. constructor.
  intros. apply H. exists (C 2). constructor.
Qed.
...
Lemma value_not_same_as_normal_form :
  exists v, value v /\ ~ normal_form step v.
Proof.
  unfold normal_form. unfold not. exists (C 2).
  split. constructor. intros.
  remember (P (C 2) (C 0)) as t.
  apply H. exists t. rewrite Heqt. 
  apply ST_Funny.
Qed.
...
(* Not provable; tfalse is a value in normal form. *)

Theorem bool_step_prop1_fails : ~ bool_step_prop1.
Proof.
  unfold not. unfold bool_step_prop1.
...
Proof.
  ...
  generalize dependent y2.
  induction H; intros; subst. inversion H0.
  - reflexivity.
  - inversion H4.
...
Remove rule [ST_IfTrue]; then the fragment [(tif ttrue ttrue tfalse)] 
fails [strong_progress].
*)
(** [] *)
...
Proof.
  ...
  apply ST_Plus2. constructor. apply ST_Plus2. constructor.
  apply ST_PlusConstConst.
...
Proof.
  ...
  unfold normal_form in *. inversion P21.
  reflexivity. exfalso. apply P12. exists y; assumption.
  induction P21. exfalso. apply P22. exists y; assumption.
  assert (y0 = y).
...
Proof.
  ...
  - apply multi_step with (P t1 y).
    apply ST_Plus2. apply v1. assumption.
    apply IHmulti.
Qed.
(** [] *)
...
Proof.
  ...
  induction Hs; intros n H0; inversion H0; subst.
...
Proof.
  ...
  constructor. constructor. apply IHt1. reflexivity. apply IHt2. reflexivity.
  inversion H. reflexivity.
  inversion H. apply IHt1 in H2. apply IHt2 in H4. subst. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - (* ST_Plus1 *)
    ...
    rewrite <- H1 in H. inversion H. subst. inversion H.
  - (* ST_Plus2 *)
    inversion H1; subst. inversion H1; subst. inversion H0.
    inversion H; subst. inversion H5. inversion H5. inversion H5.
...
Theorem weak_progress : exists t,
  ~ value t /\ ~ (exists t', t ==> t').
Proof.
...
Proof.
  ...
  apply CS_Ass.
  eapply multi_step. apply CS_Par2. apply CS_SeqFinish.
  assert (n + 1 = S n) by omega. rewrite H1. apply multi_refl.
Qed.
(** [] *)
...
Proof.
  ...
  apply IHn in H. destruct H. destruct H. exists (t_update x X (S n)).
  split. eapply multi_trans. apply H. apply par_body_n__Sn. assumption.
  split. rewrite t_update_eq. reflexivity.
...
Proof.
  ...
  try (eapply multi_step; constructor; apply multi_refl).
  - (* Plus *)
    ...
    rewrite plus_comm. apply multi_refl.
  - (* Minus *)
    ...
    apply IHe2. apply multi_trans with (p, aeval st e1 - aeval st e2 :: stk).
    eapply multi_step. constructor. simpl. apply multi_refl. apply multi_refl.
  - (* Multi *)
    ...
    apply IHe2. apply multi_trans with (p, aeval st e1 * aeval st e2 :: stk).
    eapply multi_step. constructor. simpl. apply multi_refl.
    rewrite mult_comm. apply multi_refl.
Qed.
(** [] *)
