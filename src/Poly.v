...
(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?
      ...
      - [c]
(* FILL IN HERE *)
Every item type checks except for the first one.
*)
(* Check d (b a 5). *)
Check e. (* e :: forall X : Type. X -> grumble X *)
Check d. (* d :: forall X : Type. mumble -> grumble X *)
...
Theorem app_nil_r : forall (X:Type), forall l:list X,
  l ++ [] = l.
Proof.
  intros X. intros l. induction l as [| v l' IHl].
  - simpl. reflexivity.
...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A. intros l m n. induction l as [| hl tl IHl].
  - simpl. reflexivity.
  - simpl. rewrite -> IHl. reflexivity.  
Qed.
...
Proof.
  intros X. intros l1 l2. induction l1 as [| hl1 tl1 IHl1].
  - simpl. reflexivity.
...
Theorem rev_involutive : forall X : Type, forall l : list X,
  rev (rev l) = l.
Proof.
  intros X. intros l. induction l as [| hl tl IHl].
  - simpl. reflexivity.
...
(** Try answering the following questions on paper and
    ...
    - What does
      ...
      print? *)
(* Compute (combine [1;2] [false;false;true;true]) prints [(1, false];(2, false)]. *)
Compute (combine [1;2] [false;false;true;true]).
(** [] *)
...
Fixpoint fst_split {X Y : Type} (l : list (X * Y)) : (list X) :=
  match l with
  | []              => []
  | (hX, _) :: tail => hX :: fst_split tail
  end.

Fixpoint snd_split {X Y : Type} (l : list (X * Y)) : (list Y) :=
  match l with
  | [] => []
  | (_, hY) :: tail => hY :: snd_split tail
  end.

Fixpoint split {X Y : Type} 
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X :=
  let filtered     := filter test l in
  let filtered_not := filter (fun X => negb (test X)) l in
    (filtered, filtered_not).
