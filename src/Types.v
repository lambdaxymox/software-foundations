...
Example some_term_is_stuck :
  exists t, stuck t.
Proof.
  exists (tsucc ttrue). unfold stuck. unfold step_normal_form.
  unfold not. split. intros. 
  inversion H. inversion H0. subst. inversion H2.
  intros. inversion H. inversion H0. inversion H0. subst. inversion H2.
Qed.
(** [] *)
...
Proof with eauto.
  ...
  try (inversion H0; subst;
       try (solve_by_invert)); try (find_eqn); try eauto;
  try (inversion H2; subst; assert (value t1) by auto; apply value_is_nf in H1;
       unfold normal_form in *; exfalso; eauto);
  try (inversion H2; subst; inversion H; subst; try solve_by_invert;
       inversion H; subst;
       assert (value (tsucc t)) by auto; apply value_is_nf in H3;
       unfold normal_form in *; exfalso; eauto).
Qed.
(** [] *)
...
Proof with auto.
  ...
  - left. auto.
  - destruct IHHT. 
    + left. right. constructor. apply (nat_canonical t1 HT). assumption.
    + right. inversion H. exists (tsucc x). auto.
  - destruct IHHT.
    + right. assert (nvalue t1). apply (nat_canonical t1 HT). auto.
      inversion H0. exists tzero... exists t...
    + right. inversion H. exists (tpred x)...
  - destruct IHHT.
    + right. assert (nvalue t1). apply (nat_canonical t1 HT). auto.
      inversion H0. eauto. eauto. (* exists ttrue... exists tfalse... *)
...
Proof with auto.
  ...
  induction HT;
    ...
    - (* T_Succ *)
      inversion HE; subst; clear HE. constructor.
      apply IHHT; assumption.
    - (* T_Pred *)
      inversion HE; subst; clear HE. constructor. subst.
      inversion HT. subst. assumption. apply IHHT in H0.
      constructor. assumption.
    - inversion HE; subst. constructor. constructor. eauto.
Qed.
(** [] *)
...
Proof with eauto.
  ...
  induction HT;
      ...
      try solve_by_invert.
  - (* T_If *)
    inversion HE; subst. assumption. assumption. 
    constructor. apply IHHT1. assumption. assumption. assumption.
  - (* T_Succ *)
    inversion HE; subst...
  - (* T_Pred *)
    inversion HE; subst... inversion HT...
  - (* T_Iszero *)
    inversion HE; subst...
Qed.
(** [] *)
...
Proof with eauto.
  ...
  eapply H; auto. solve_by_inverts 2.
Qed.
(** [] *)
