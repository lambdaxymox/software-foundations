...
Proof.
  assert (Ht : forall a1 : aexp, aeval (optimize_0plus a1) = aeval a1).
...
(* This version of the proof goes for maximum compactness using the
   [try] tactical (combinator?). *)
Theorem optimize_0plus_b_sound' : forall b,
  beval (optimize_0plus_b b) = beval b.
Proof.
  assert (Ht : forall a1 : aexp, aeval (optimize_0plus a1) = aeval a1).
  { apply optimize_0plus_sound. }
  intros b. 
  induction b ; simpl ; try (repeat (rewrite -> Ht)) ; reflexivity.
Qed.
(** [] *)
...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BEq e1 e2 n1 n2 : aevalR e1 n1 -> aevalR e2 n2 -> bevalR (BEq e1 e2) (beq_nat n1 n2)
  | E_BLe e1 e2 n1 n2 : aevalR e1 n1 -> aevalR e2 n2 -> bevalR (BLe e1 e2) (leb n1 n2) 
  | E_BNot e1 b1        : bevalR e1 b1 -> bevalR (BNot e1) (negb b1)
...
Proof.
  ...
  - generalize dependent bv.
    induction b.
    + simpl. intros. subst. apply E_BTrue. 
    + simpl. intros. subst. apply E_BFalse.
    + simpl. intros. subst. apply E_BEq. 
      apply aeval_iff_aevalR. reflexivity.
      apply aeval_iff_aevalR. reflexivity.
    + simpl. intros. subst. constructor.
      apply aeval_iff_aevalR. reflexivity.
      apply aeval_iff_aevalR. reflexivity.
    + simpl. intros. subst. constructor. apply IHb. reflexivity.
    + simpl. intros. subst. constructor. apply IHb1. reflexivity.
...
Proof.
  ...
  apply E_Ass. auto.
  apply E_WhileLoop with (t_update (t_update (t_update (t_update empty_state X 2) Y 0) Y 2) X 1). auto.
