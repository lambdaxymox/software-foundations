...
Proof.
  intros n m. intro H. apply and_intro.
  - induction n.
    + reflexivity.
    + inversion H.
  - induction n.
    + assert(H0 : 0 + m = m). { auto. }
      rewrite <- H0. apply H.
    + inversion H.
Qed.
(** [] *)
...
Fact not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P contra Q HP. unfold not in HP. apply contra in HP. 
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q HPQ HQ. unfold not in HQ. unfold not. intros HP. 
  apply HPQ in HP as HQ'. destruct HQ. apply HQ'.
Qed.
(** [] *)
...
Proof.
  ...
  - generalize l, y. induction l0.
    ...
    + simpl. intros y0 [H | H]. exists x. split. assumption. 
      * left. reflexivity. 
      * apply IHl0 in H. destruct H. exists x0. split.
